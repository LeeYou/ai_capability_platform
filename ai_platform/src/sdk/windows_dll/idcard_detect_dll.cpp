...
std::unique_ptr<ai_platform::IdCardDetectSdk> g_sdk;
...
AiIdCardDetectDllResult make_result(int code, const std::string& result_json, const std::string& error_message) {
    g_result_json = result_json;
    g_error_message = error_message;
    AiIdCardDetectDllResult result{};
    result.code = code;
...
extern "C" AI_SDK_EXPORT int ai_idcard_detect_sdk_version() {
    return 1;
}

extern "C" AI_SDK_EXPORT int ai_idcard_detect_initialize(const char* plugin_path, const char* model_dir, const char* license_path) {
    g_sdk = std::make_unique<ai_platform::IdCardDetectSdk>();
...
extern "C" AI_SDK_EXPORT const char* ai_idcard_detect_capability_id() {
    return g_capability_id.c_str();
}

extern "C" AI_SDK_EXPORT const char* ai_idcard_detect_last_license_failure_reason() {
    return g_license_failure_reason.empty() ? nullptr : g_license_failure_reason.c_str();
}

extern "C" AI_SDK_EXPORT const char* ai_idcard_detect_last_license_failure_detail() {
    return g_license_failure_detail.empty() ? nullptr : g_license_failure_detail.c_str();
}

extern "C" AI_SDK_EXPORT AiIdCardDetectDllResult ai_idcard_detect_infer_base64(const char* image_base64, const char* image_format) {
    if (!g_sdk) {
        return make_result(-1, "", "sdk not initialized");
    }

    const auto infer_result = g_sdk->infer_image_base64(image_base64 ? image_base64 : "", image_format ? image_format : "jpg");
    return make_result(infer_result.code, infer_result.result_json, infer_result.ok ? std::string() : infer_result.error_message);
}

extern "C" AI_SDK_EXPORT void ai_idcard_detect_shutdown() {
    g_sdk.reset();
    g_result_json.clear();
    g_error_message.clear();
