...
std::string prepare_license(const std::string& capability_id) {
    ...
    data.license_id = "LIC-DLL-001";
    data.version = "1.0";
    data.license_type = "development";
    data.customer_id = "CUST-DLL-001";
...
std::string prepare_invalid_signature_license(const std::string& capability_id) {
    ...
    ai_platform::LicenseFileData data;
    data.license_id = "LIC-DLL-001-INVALID";
    data.version = "1.0";
    data.license_type = "development";
    data.customer_id = "CUST-DLL-001";
    data.customer_name = "demo_customer";
...
int main(int argc, char** argv) {
    assert_true(argc >= 3, "usage: test_face_detect_dll <plugin_path> <model_dir>");
    const auto license_path = prepare_license("face_detect");
    assert_true(ai_face_detect_initialize(argv[1], argv[2], license_path.c_str()) == 0, "dll initialize should succeed");
    assert_true(std::string(ai_face_detect_capability_id()) == "face_detect", "dll capability id should be face_detect");

    const auto result = ai_face_detect_infer_base64("YWJjZA==", "jpg");
    ...
    assert_true(std::string(result.result_json).find("\"mock\":true") != std::string::npos, "dll infer should contain mock marker");

    ai_face_detect_shutdown();

    const auto invalid_license_path = prepare_invalid_signature_license("face_detect");
    assert_true(ai_face_detect_initialize(argv[1], argv[2], invalid_license_path.c_str()) == -1, "dll initialize should fail for invalid license");
    assert_true(ai_face_detect_last_license_failure_reason() != nullptr, "dll invalid license failure reason should exist");
    assert_true(std::string(ai_face_detect_last_license_failure_reason()) == ai_platform::kLicenseFailureReasonSignatureInvalid, "dll invalid license failure reason should be signature_invalid");
    assert_true(ai_face_detect_last_license_failure_detail() != nullptr, "dll invalid license failure detail should exist");
    assert_true(std::string(ai_face_detect_last_license_failure_detail()) == "license signature invalid", "dll invalid license failure detail should expose signature detail");
    const auto invalid_result = ai_face_detect_infer_base64("YWJjZA==", "jpg");
