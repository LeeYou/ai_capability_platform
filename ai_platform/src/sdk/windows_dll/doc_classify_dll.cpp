...
std::unique_ptr<ai_platform::DocClassifySdk> g_sdk;
...
AiDocClassifyDllResult make_result(int code, const std::string& result_json, const std::string& error_message) {
    g_result_json = result_json;
    g_error_message = error_message;
    AiDocClassifyDllResult result{};
    result.code = code;
...
extern "C" AI_SDK_EXPORT int ai_doc_classify_sdk_version() {
    return 1;
}

extern "C" AI_SDK_EXPORT int ai_doc_classify_initialize(const char* plugin_path, const char* model_dir, const char* license_path) {
    g_sdk = std::make_unique<ai_platform::DocClassifySdk>();
...
extern "C" AI_SDK_EXPORT const char* ai_doc_classify_capability_id() {
    return g_capability_id.c_str();
}

extern "C" AI_SDK_EXPORT const char* ai_doc_classify_last_license_failure_reason() {
    return g_license_failure_reason.empty() ? nullptr : g_license_failure_reason.c_str();
}

extern "C" AI_SDK_EXPORT const char* ai_doc_classify_last_license_failure_detail() {
    return g_license_failure_detail.empty() ? nullptr : g_license_failure_detail.c_str();
}

extern "C" AI_SDK_EXPORT AiDocClassifyDllResult ai_doc_classify_infer_base64(const char* image_base64, const char* image_format) {
    if (!g_sdk) {
...
extern "C" AI_SDK_EXPORT void ai_doc_classify_shutdown() {
    g_sdk.reset();
    g_result_json.clear();
    g_error_message.clear();
