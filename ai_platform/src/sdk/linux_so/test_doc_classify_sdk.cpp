...
std::string prepare_license(const std::string& capability_id) {
    ...
    data.license_id = "LIC-SDK-004";
    data.version = "1.0";
    data.license_type = "development";
    data.customer_id = "CUST-SDK-004";
...
std::string prepare_denied_license(const std::string& capability_id) {
    ...
    ai_platform::LicenseFileData data;
    data.license_id = "LIC-SDK-004-DENIED";
    data.version = "1.0";
    data.license_type = "development";
    data.customer_id = "CUST-SDK-004";
    data.customer_name = "demo_customer";
...
int main(int argc, char** argv) {
    assert_true(argc >= 3, "usage: test_doc_classify_sdk <plugin_path> <model_dir>");
    ...
    assert_true(sdk.capability_id() == "doc_classify", "sdk capability id should be doc_classify");
    ...
    assert_true(infer_result.result_json.find("\"document_type\":\"id_card\"") != std::string::npos, "sdk infer result should contain document type field");

    ai_platform::DocClassifySdk denied_sdk;
    const auto denied_license_path = prepare_denied_license("doc_classify");
    ...
    assert_true(denied_sdk.last_license_failure_detail() == "doc_classify", "sdk denied failure detail accessor should expose capability id");
