#ifndef AI_PLATFORM_DOC_CLASSIFY_SDK_H
#define AI_PLATFORM_DOC_CLASSIFY_SDK_H
...
using DocClassifySdkResult = CapabilitySdkResult;

class DocClassifySdk {
public:
    DocClassifySdk();
    ~DocClassifySdk();

    bool initialize(const std::string& library_path, const std::string& model_dir, const std::string& license_path = std::string());
    DocClassifySdkResult infer_image_base64(const std::string& image_base64, const std::string& image_format) const;
