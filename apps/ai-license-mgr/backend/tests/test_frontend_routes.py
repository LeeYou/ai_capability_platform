...
class FrontendRoutesTestCase(unittest.TestCase):
    def setUp(self) -> None:
        ...
        (self.frontend_dist / "index.html").write_text("<html><body>ai-license-mgr</body></html>", encoding="utf-8")
        (self.frontend_dist / "assets" / "app.js").write_text("console.log('ai-license-mgr')", encoding="utf-8")
    ...
    def test_frontend_routes_are_registered_without_breaking_api_routes(self) -> None:
        ...
        self.assertEqual(Path(root_response.path).read_text(encoding="utf-8"), "<html><body>ai-license-mgr</body></html>")

        frontend_route = next(route for route in app.routes if route.path == "/{frontend_path:path}")
        nested_response = frontend_route.endpoint("license/workbench")
        self.assertEqual(Path(nested_response.path).read_text(encoding="utf-8"), "<html><body>ai-license-mgr</body></html>")
