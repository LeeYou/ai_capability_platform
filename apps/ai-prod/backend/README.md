...
7. `/api/v1/infer/{capability_name}` 已由 C++ 直接完成请求解析、license quick check、设备选择、实例池借还、真实插件动态加载执行、结果生成与基础日志审计；runtime snapshot 不可用时直接返回运行时错误
8. `reload/rollback` 已由 C++ 直接完成资源扫描、revision/operation SQLite 持久化、runtime snapshot 重写，以及实例池 drain 编排与切换后 catalog/pool 刷新
9. `license/status` 已改为由 C++ 直接读取标准 license 文件，`infer` 与 `reload/rollback` 已接入 license quick check
10. 已新增 `/api/v1/admin/license-reload`，支持 C++ 侧 license 手动重载与自动监测刷新
11. Python runtime 的 bootstrap / reload / rollback 逻辑仅保留用于内部单元测试与验收行为对照，不再通过公开 `/api/v1/*` 暴露
12. `/internal/*` 查询接口仅保留在 Python 测试验收外壳侧，不再通过 C++ 生产主链路暴露
13. runtime snapshot / capability catalog 已补齐 `model_root`、`binary_path` 元数据，供 C++ infer 侧直接装载并调用插件
14. 启动阶段已支持在 runtime snapshot 缺失时由 C++ 直接完成资源扫描、license 校验、bootstrap revision 持久化与 snapshot 初始写入
15. 当前已新增 runtime 显式状态机，`/api/v1/admin/catalog` 可输出 `bootstrapping / ready / draining / transitioning / error` 状态与错误信息
16. `reload/rollback` 已增加切换互斥保护，并发管理操作会直接拒绝，避免运行时状态竞争
17. 当前已新增请求级 in-flight 跟踪与 RAII 请求租约，`/api/v1/admin/catalog` 可输出 active request 明细，drain 阶段会等待活动请求清空后再执行切换
18. 当前已补齐能力级 `execution_metrics` 与 `plugin_info` 观测字段，`/api/v1/admin/catalog` 可输出请求量、成功/失败次数、推理耗时及当前插件元数据
19. 当前已补齐可选 `warmup` / `health_check` 生命周期钩子，相关状态、时间戳与失败信息会并入 capability 级 `execution_metrics`
20. 当前已新增统一输入 `payload codec`，`image / video / pdf` 请求会在 C++ 侧完成 base64 解码、格式校验，并将 `input_metadata` 回填到推理结果与审计日志
21. 当前已补齐基于 revision 的能力资源快照持久化；rollback 会优先恢复目标 revision 的具体模型目录、插件目录、插件文件与 manifest 元数据，而不是仅按能力名重扫当前目录
