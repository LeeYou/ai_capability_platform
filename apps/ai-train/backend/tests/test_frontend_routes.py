...
class FrontendRoutesTestCase(unittest.TestCase):
    ...
    def test_frontend_root_and_assets_are_served_without_breaking_api_routes(self) -> None:
        with TestClient(create_app()) as client:
            ...
            self.assertIn("ai-train", root_response.text)

            nested_response = client.get("/annotation/workbench")
            self.assertEqual(nested_response.status_code, 200)
            self.assertIn("ai-train", nested_response.text)
            ...
            self.assertEqual(health_response.json()["service"], "ai-train")
