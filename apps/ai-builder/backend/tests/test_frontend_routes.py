...
class FrontendRoutesTestCase(unittest.TestCase):
    ...
    def test_frontend_routes_are_registered_without_breaking_api_routes(self) -> None:
        ...
        self.assertEqual(Path(root_response.path).read_text(encoding="utf-8"), "<html><body>ai-builder</body></html>")

        frontend_route = next(route for route in app.routes if route.path == "/{frontend_path:path}")
        nested_response = frontend_route.endpoint("build/workbench")
        self.assertEqual(Path(nested_response.path).read_text(encoding="utf-8"), "<html><body>ai-builder</body></html>")
