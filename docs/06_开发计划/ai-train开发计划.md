...
| T12 | 标注 schema 分层与任务类型标准化 | 未开始 |
| T13 | 标注结果到训练输入适配器链路收敛 | 未开始 |
| T14 | 真实训练执行器与模型导出链路收敛 | 未开始 |
| T15 | 模型包 manifest 与 runtime / builder 消费契约收敛 | 未开始 |
| T16 | 新增能力训练模板 / 测试模板 / 推理模板统一脚手架 | 未开始 |
...
1. T12：标注 schema 分层与任务类型标准化。
2. T13：标注结果到训练输入适配器链路收敛。
3. T14：真实训练执行器与模型导出链路收敛。
4. T15：模型包 manifest 与 runtime / builder 消费契约收敛。
5. T16：新增能力训练模板 / 测试模板 / 推理模板统一脚手架。

### 4.4 阶段小结

ai-train 上一轮已完成 T9/T10/T11，具备标注台、训练台、模型包元数据与交付 manifest 基础能力；但结合本轮逻辑自洽审查，仍需继续完成标注 schema 分层、标注结果到训练输入适配、真实训练执行器、模型包与 runtime / builder 强契约，以及新增能力模板化脚手架等工作。因此本模块计划已进入新一轮整改阶段，新增 T12-T16 作为后续逐项实施与跟踪基线。
