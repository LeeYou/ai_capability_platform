...
5. 首页需要补齐运行健康度、阻塞项、重点能力、最近变更与审计摘要，形成内部运行中枢。
6. 在线验证页需要在执行前展示能力上下文、授权上下文与载荷门禁，避免“先调用再补核对”。
7. 版本控制页需要把 revision 选择、变更门禁、风险摘要、source_summary 与审计联动到同一视图。
8. 监控诊断页需要把 queue timeout、busy reject、P95 峰值与 admission/source_summary 失败信息聚合为可行动诊断面板。
9. 运行状态页需要将能力资产、授权态势、revision 与资源池利用率合并展示，形成可验收的运行资产视图。
