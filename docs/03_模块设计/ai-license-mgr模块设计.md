...
5. 首页需要补齐到期提醒、工具契约、稳定结果码摘要与最近审计留痕，成为授权中枢页。
6. 连续签发页需要同时展示签发门禁、策略预览和签发载荷快照，减少“签发后再回看”的跳转成本。
7. 风险操作页需要展示轮转迁移结果、关联策略/签发记录与审计确认视图，确保高风险动作有完整留痕。
8. 客户页需要从基础资料列表升级为客户组合视图，能够看到客户下的策略、签发和应用资产沉淀情况。
9. 策略页需要同时展示环境约束、到期风险、密钥状态与关联签发记录，避免策略与签发资产脱节。
